...
def search_docs(
    ...
    kb = KBServiceFactory.get_service_by_name(knowledge_base_name)  # 得到知识库对象
    data = []
    if kb is not None:
        if query:
            docs = kb.search_docs(query, top_k, score_threshold)
            # 转换成DocumentWithVSId(比Document增加了id,score两个属性)
            data = [
                DocumentWithVSId(**x[0].dict(), score=x[1], id=x[0].metadata.get("id"))
                for x in docs
            ]
    ...
    index = 1
    for doc in data:
        # 打印文档内容,输出编号,把输出中的回车换行替换成空格
        text = doc.page_content.replace("\n", "\\n")
        print(f"#{index} 分数:{doc.score} 来源:{doc.metadata['source']}:\n{text}")
        index += 1
